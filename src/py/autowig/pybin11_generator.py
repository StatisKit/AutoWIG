...
__all__ = ['pybind11_call_policy', 'pybind11_export', 'pybind11_module', 'pybind11_decorator']
...
def set_pybind11_export(self, pybind11_export):
    ...
    if isinstance(pybind11_export, BoostPythonExportFileProxy):
        scope = pybind11_export.scope
        if scope and not scope == self.parent:
            raise ValueError()
    elif not isinstance(pybind11_export, bool):
        raise TypeError('\'pybind11_export\' parameter must be boolean, a \'' + BoostPythonExportFileProxy.__class__.__name__ + '\' instance or identifer')
...
def _valid_pybind11_export(self):
    if self.pybind11_call_policy == 'boost::python::return_value_policy< boost::python::reference_existing_object >()':
...
def _valid_pybind11_export(self):
    if self.pybind11_call_policy in ['boost::python::return_internal_reference<>()',
...
% if not cls.is_error:
    % for method in cls.methods(access='public'):
        % if method.pybind11_export and method.is_overloaded:
    ...
    def edit(self, row):
        ...
        with open(self.globalname, 'r') as filehandler:
            ...
            if not row == len(lines):
                        ...
                        if parsed:
                            ...
                            if parsed:
                                if 'what' not in parsed.named:
                                    ...
                                    return "asg['" + parsed['cpp'].lstrip('&') + "'].pybind11_export = False"
                            else:
                ...
                return ('\n'.join("asg['" + declaration.globalname + "'].pybind11_export = False" for declaration in self.declarations if isinstance(declaration, ClassProxy))
                        + "\nif '" + self.globalname + "' in asg:\n\tasg['" + self.globalname + "'].remove()\n")
        return ""
...
pybind11_MODULE(_${module.prefix})
{
% for export in module.exports:
    % if not export.is_empty:
    ${export.prefix}();
