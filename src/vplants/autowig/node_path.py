...
class FlatNodePathPlugin(object):
    ...
    def implementation(self, node, prefix='', suffix=''):
        ...
        if node.globalname == '::':
            ...
            if isinstance(node, ClassTemplateSpecializationProxy):
                filepath = self.implementation(node.specialize, prefix=prefix, suffix=suffix)
            else:
                filepath = self.implementation(node.parent, prefix=prefix, suffix='') + '_' + camel_case_to_lower(node.localname) + suffix
